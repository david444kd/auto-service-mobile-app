      ...
      {services.map((service) => {
        ...
        return (
          <TouchableOpacity key={service.id} onPress={() => onSelect(service)} activeOpacity={0.75}>
            <Card variant={isSelected ? 'elevated' : 'default'} style={isSelected ? { borderWidth: 1.5, borderColor: '#f59e0b' } : undefined}>
              <View className="flex-row items-center gap-3">
                <View className={`h-10 w-10 items-center justify-center rounded-xl ${isSelected ? 'bg-amber-500/20' : 'bg-[#0e0f12]'}`}>
